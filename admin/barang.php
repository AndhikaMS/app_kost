...
if (isset($_GET['hapus'])) {
    ...
    if ($id > 0) {
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            $pesan = '<div class="alert alert-danger">Gagal menghapus barang.</div>';
        }
    }
}
...
if (isset($_GET['msg']) && $_GET['msg'] === 'sukses') {
    $pesan = '<div class="alert alert-success">Barang berhasil ditambahkan.</div>';
}
if (isset($_GET['msg']) && $_GET['msg'] === 'edit_sukses') {
    $pesan = '<div class="alert alert-success">Barang berhasil diedit.</div>';
}
if (isset($_GET['msg']) && $_GET['msg'] === 'hapus_sukses') {
    $pesan = '<div class="alert alert-success">Barang berhasil dihapus.</div>';
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>Kelola Barang - Admin Kos</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="container py-5">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                <?php
                $sql = "SELECT * FROM tb_barang ORDER BY nama ASC";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo '<td>';
                        echo '<a href="barang_edit.php?id=' . $row['id'] . '" class="btn btn-sm btn-warning">Edit</a> ';
                        echo '<a href="?hapus=' . $row['id'] . '" class="btn btn-sm btn-danger" onclick="return confirmHapus(\'' . htmlspecialchars($row['nama']) . '\')">Hapus</a>';
                        echo '</td>';
                        echo '</tr>';
                    }
                } else {
                    echo '<tr><td colspan="4" class="text-center">Belum ada data barang.</td></tr>';
                }
                ?>
            </tbody>
        </table>

<?php
include '../inc/db.php';
include '../inc/functions.php';

$pesan = '';

// Proses tambah pembayaran
if (isset($_POST['tambah_bayar'])) {
    $id_tagihan = (int) $_POST['id_tagihan'];
    $jml_bayar = (int) $_POST['jml_bayar'];
    $tgl_bayar = mysqli_real_escape_string($conn, $_POST['tgl_bayar']);
    $status = mysqli_real_escape_string($conn, $_POST['status']);
    
    // Validasi input
    if ($id_tagihan > 0 && $jml_bayar > 0 && $tgl_bayar) {
        // Cek apakah tagihan ada
        $check_sql = "SELECT t.jml_tagihan, COALESCE(SUM(b.jml_bayar), 0) as total_bayar
                      FROM tb_tagihan t
                      LEFT JOIN tb_bayar b ON t.id = b.id_tagihan
                      WHERE t.id = $id_tagihan
                      GROUP BY t.id";
        $check_result = mysqli_query($conn, $check_sql);
        
        if (mysqli_num_rows($check_result) > 0) {
            $check_row = mysqli_fetch_assoc($check_result);
            $total_bayar = $check_row['total_bayar'] + $jml_bayar;
            
            // Update status berdasarkan total pembayaran
            if ($total_bayar >= $check_row['jml_tagihan']) {
                $status = 'lunas';
            } else {
                $status = 'cicil';
            }
            
            $sql = "INSERT INTO tb_bayar (id_tagihan, jml_bayar, tgl_bayar, status) VALUES ($id_tagihan, $jml_bayar, '$tgl_bayar', '$status')";
            if (mysqli_query($conn, $sql)) {
                header('Location: bayar.php?msg=sukses');
                exit;
            } else {
                $pesan = '<div class="alert alert-danger">Gagal menambah pembayaran.</div>';
            }
        } else {
            $pesan = '<div class="alert alert-warning">Tagihan tidak ditemukan.</div>';
        }
    } else {
        $pesan = '<div class="alert alert-warning">Semua field harus diisi dengan benar.</div>';
    }
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>Tambah Pembayaran - Admin Kos</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container-fluid">
            <a class="navbar-brand" href="dashboard.php">Admin Kos</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link" href="dashboard.php">Dashboard</a></li>
                    <li class="nav-item"><a class="nav-link" href="kamar.php">Kamar</a></li>
                    <li class="nav-item"><a class="nav-link" href="penghuni.php">Penghuni</a></li>
                    <li class="nav-item"><a class="nav-link" href="kmr_penghuni.php">Data Hunian</a></li>
                    <li class="nav-item"><a class="nav-link" href="brng_bawaan.php">Barang Bawaan</a></li>
                    <li class="nav-item"><a class="nav-link" href="barang.php">Barang</a></li>
                    <li class="nav-item"><a class="nav-link" href="tagihan.php">Tagihan</a></li>
                    <li class="nav-item"><a class="nav-link active" href="bayar.php">Pembayaran</a></li>
                </ul>
            </div>
        </div>
    </nav>
    <div class="container py-5">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">Tambah Pembayaran Baru</h4>
                    </div>
                    <div class="card-body">
                        <?php echo $pesan; ?>
                        
                        <form method="POST">
                            <div class="mb-3">
                                <label for="id_tagihan" class="form-label">Pilih Tagihan</label>
                                <select class="form-control" id="id_tagihan" name="id_tagihan" required>
                                    <option value="">Pilih Tagihan</option>
                                    <?php
                                    // Jika ada parameter tagihan, pre-select tagihan tersebut
                                    $selected_tagihan = isset($_GET['tagihan']) ? (int)$_GET['tagihan'] : 0;
                                    ?>
                                    <?php
                                    $sql = "SELECT t.id, t.bulan, t.jml_tagihan,
                                                   p.nama as nama_penghuni,
                                                   km.nomor as nomor_kamar,
                                                   COALESCE(SUM(b.jml_bayar), 0) as total_bayar
                                            FROM tb_tagihan t
                                            JOIN tb_kmr_penghuni kp ON t.id_kmr_penghuni = kp.id
                                            JOIN tb_penghuni p ON kp.id_penghuni = p.id
                                            JOIN tb_kamar km ON kp.id_kamar = km.id
                                            LEFT JOIN tb_bayar b ON t.id = b.id_tagihan
                                            GROUP BY t.id
                                            HAVING total_bayar < t.jml_tagihan
                                            ORDER BY t.bulan DESC, p.nama ASC";
                                    $result = mysqli_query($conn, $sql);
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $sisa = $row['jml_tagihan'] - $row['total_bayar'];
                                        $selected = ($row['id'] == $selected_tagihan) ? 'selected' : '';
                                        echo '<option value="' . $row['id'] . '" data-tagihan="' . $row['jml_tagihan'] . '" data-bayar="' . $row['total_bayar'] . '" ' . $selected . '>';
                                        echo format_bulan($row['bulan']) . ' - ' . htmlspecialchars($row['nama_penghuni']) . ' (Kamar ' . htmlspecialchars($row['nomor_kamar']) . ') - Sisa: Rp ' . number_format($sisa, 0, ',', '.');
                                        echo '</option>';
                                    }
                                    ?>
                                </select>
                                <div class="form-text">Pilih tagihan yang belum lunas</div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="jml_bayar" class="form-label">Jumlah Bayar</label>
                                <div class="input-group">
                                    <span class="input-group-text">Rp</span>
                                    <input type="number" class="form-control" id="jml_bayar" name="jml_bayar" 
                                           placeholder="Masukkan jumlah pembayaran" required>
                                </div>
                                <div class="form-text">Jumlah pembayaran untuk tagihan ini</div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="tgl_bayar" class="form-label">Tanggal Pembayaran</label>
                                <input type="date" class="form-control" id="tgl_bayar" name="tgl_bayar" 
                                       value="<?php echo date('Y-m-d'); ?>" required>
                                <div class="form-text">Tanggal pembayaran dilakukan</div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="status" class="form-label">Status</label>
                                <select class="form-control" id="status" name="status" required>
                                    <option value="cicil">Cicil</option>
                                    <option value="lunas">Lunas</option>
                                </select>
                                <div class="form-text">Status akan diupdate otomatis berdasarkan jumlah pembayaran</div>
                            </div>
                            
                            <div class="d-flex justify-content-between">
                                <a href="bayar.php" class="btn btn-secondary">Kembali</a>
                                <button type="submit" name="tambah_bayar" class="btn btn-success">Simpan Pembayaran</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
    // Auto-update status berdasarkan jumlah pembayaran
    function updatePaymentInfo() {
        const selectedOption = document.getElementById('id_tagihan').options[document.getElementById('id_tagihan').selectedIndex];
        const tagihan = selectedOption.getAttribute('data-tagihan');
        const bayar = selectedOption.getAttribute('data-bayar');
        
        if (tagihan && bayar) {
            const sisa = tagihan - bayar;
            document.getElementById('jml_bayar').max = sisa;
            document.getElementById('jml_bayar').placeholder = 'Maksimal: Rp ' + sisa.toLocaleString('id-ID');
        }
    }
    
    document.getElementById('id_tagihan').addEventListener('change', updatePaymentInfo);
    
    // Auto-fill jika ada tagihan yang dipilih
    if (document.getElementById('id_tagihan').value) {
        updatePaymentInfo();
    }
    
    document.getElementById('jml_bayar').addEventListener('input', function() {
        const selectedOption = document.getElementById('id_tagihan').options[document.getElementById('id_tagihan').selectedIndex];
        const tagihan = selectedOption.getAttribute('data-tagihan');
        const bayar = selectedOption.getAttribute('data-bayar');
        
        if (tagihan && bayar) {
            const totalBayar = parseInt(bayar) + parseInt(this.value || 0);
            const statusSelect = document.getElementById('status');
            
            if (totalBayar >= tagihan) {
                statusSelect.value = 'lunas';
            } else {
                statusSelect.value = 'cicil';
            }
        }
    });
    </script>
</body>
</html> 